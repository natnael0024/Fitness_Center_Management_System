...
// window.darkMode = function (el) {
  ...
  // Toggle the dark mode state and store in localStorage
  if (localStorage.getItem('gymdarkMode') === 'true') {
    console.log("HDFYDHWJD")
    applyDarkMode();
    el.setAttribute("checked", "true");
  } else {
    applyLightMode();
  ...
  el.addEventListener('change', function () {
    if (el.checked) {
      localStorage.setItem('gymdarkMode', 'true'); // Save dark mode preference
      applyDarkMode();  // Apply dark mode immediately
    } else {
      localStorage.setItem('gymdarkMode', 'false'); // Save light mode preference
      applyLightMode();  // Apply light mode immediately
    }
  });
